//ќтсотрировать стобцы матрицы по возрастанию и убыванию значений элементов.
...
public static int[][] massiveCreate2 (int n,int m){
	...
	int [][]a=new int[n[0].length][n.length];	
		for(int i=0;i<n.length;i++){
		for (int j=0;j<n[0].length;j++){
		a[j][i]=n[i][j];
		}
...
System.out.print("¬ведите количество строк  ");
	...
	print(a);
	System.out.println("-----------");
	print(reverse(function(reverse(a))));
	System.out.println("-----------");
	print(reverse(function1(reverse(a))));
	}
}
}
